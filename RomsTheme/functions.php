...
/**
 * --------------------------------
 * 1. REGISTRO DE UBICACIONES DE MENÚS
 * --------------------------------
 * Crea las ubicaciones de menú para que aparezcan en el panel de WordPress 
 * (Apariencia > Menús > Gestionar ubicaciones).
 */
if (!function_exists('romsps2_register_nav_menus')) {
    function romsps2_register_nav_menus() {
        register_nav_menus( array(
            'footer_menu_pages'      => esc_html__( 'Páginas del Footer', 'romsps2' ),
            'footer_menu_categories' => esc_html__( 'Categorías del Footer', 'romsps2' ),
        ) );
    }
    // Asegura que la función se ejecute cuando el tema esté listo.
    add_action( 'after_setup_theme', 'romsps2_register_nav_menus' );
}


/**
 * --------------------------------
 * 2. FUNCIÓN DE FALLBACK PARA EL FOOTER
 * --------------------------------
 * Esta función es llamada por wp_nav_menu en footer.php 
 * si no se ha asignado ningún menú a la ubicación.
 */
if (!function_exists('romsps2_footer_menu_fallback')) {
    function romsps2_footer_menu_fallback($args) {
        ...
        if (!has_nav_menu($args['theme_location'])) {
            ...
            // Adaptamos los enlaces de ejemplo según la ubicación
            if ($args['theme_location'] === 'footer_menu_pages') {
                $menu_html .= '<li><a href="#">Sobre Nosotros (Ejemplo)</a></li>';
                $menu_html .= '<li><a href="#">Contacto (Ejemplo)</a></li>';
                $menu_html .= '<li><a href="#">Política de Privacidad (Ejemplo)</a></li>';
            } elseif ($args['theme_location'] === 'footer_menu_categories') {
                $menu_html .= '<li><a href="#">Acción (Ejemplo)</a></li>';
                $menu_html .= '<li><a href="#">RPG (Ejemplo)</a></li>';
                $menu_html .= '<li><a href="#">Deportes (Ejemplo)</a></li>';
            }
            $menu_html .= '</ul>';
            echo $menu_html;
        }
